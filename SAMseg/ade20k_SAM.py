...
def setup_distributed():
    ...
    os.environ['MASTER_PORT'] = '12398'  # 更改为未被使用的端口
...
class ADE20KDataset(Dataset):
    def __init__(self, root, transform=None, mode='training'):  # validation
        self.root = root
...
def save_results(combined_mask, img_path, mask_save_dir, label_save_dir):
    combined_mask_file_name = os.path.basename(img_path).replace('.jpg', '.json')
    ...
    label_file_name = os.path.basename(img_path).replace('.jpg', '.png')
...
def main():
    setup_distributed()
    data_dir = "/data/ADE20K/ade20k/ADEChallengeData2016/"
    mask_save_dir = "/data/ade20k_SAM/val_ade20k_sam/ade20k_SAM_mask/"
    label_save_dir = "/data/ade20k_SAM/val_ade20k_sam/ade20k_SAM/"
    ...
    dataset = ADE20KDataset(data_dir, mode='validation')  # validation  training
...
# CUDA_VISIBLE_DEVICES=2 python -m torch.distributed.launch --nproc_per_node=1 ade20k_SAM.py
