...
class BlockFusion(nn.Module):
    def __init__(self, dim):
        ...
        # 创建投影块，将两个块的维度减少到一个块的维度
        self.proj_blocks = self._make_proj_block(dim * 2, dim)
    ...
    def _proj_block_operations(self, x, cls_token, proj_block):
        # 将 cls_token 和 x 连接起来，然后通过投影块
        cls_x = torch.cat([cls_token, x], dim=1)
        cls_x = proj_block(cls_x).type_as(x)
        cls_token, x = cls_x[:, :1, :], cls_x[:, 1:, :]
        return x, cls_token

    def forward(self, block1, block2, cls_token1, cls_token2):
        # 将两个块和它们的 cls_token 进行连接
        out_block = torch.cat([block1, block2], dim=-1)
        out_cls_token = torch.cat([cls_token1, cls_token2], dim=-1)
        # 通过投影块处理连接后的输出
        out_block, out_cls_token = self._proj_block_operations(out_block, out_cls_token, self.proj_blocks)
...
#         )
#
#     def _proj_block_operations(self, x, proj_block):
#         # 将 cls_token 和 x 连接起来，然后通过投影块
#         x = proj_block(x).type_as(x)
#         return x
#
...
#         out_block = torch.cat([block1, block2], dim=-1)
#         # 通过投影块处理连接后的输出
#         out_block = self._proj_block_operations(out_block, self.proj_blocks)
#         return out_block